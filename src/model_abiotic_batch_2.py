...
df = df_a  #setting wroking df as aboitic one in the full data set 

#for stats of data calulation
...
#setting sigma with values from data error structure analyses
df['log_sigma'] = 0.2
...
## Reading in inits files for 0 model
inits0 = pd.read_csv("../data/inits/abiotic_control_2.csv")
...
#find closest time  of model to data and save in mod
...
#setting H mean for odelib search  via inits file 
H0_mean = inits0['H0'][0]


# number of iterations for MCMC - INCREASE FOR MORE BELL CURVEY LOOKING HISTS
...
fig1.savefig('../figures/abiotic2_0_dynamics')
...
fig2.savefig('../figures/abiotic2_0_params')


#################################
#graphing trace plots for parameter values
##################################
...
#print out plot
fig3.savefig('../figures/abiotic2_0_TRACE')
...
plt.show()
fig4.savefig('../figures/abiotic2_0_residuals')




pframe0 = pd.DataFrame(a0.get_parameters(),columns=a0.get_pnames())
pframe0.to_csv("../data/inits/abiotic_control_2.csv")
