...
#df0 = df[df['id'] =='CatMutsNoC_growth'] #set id of assay we are fitting
df0 = df[df['id'] =='CatMutsNoC_stationary'] #set id of assay we are fitting

  #assay 0 H 
df15 = df[df['id'] =='CatMutsNoC_Hspike'] #assay of spiked HOOH


st  = 'WT'

#st  = 'deltaOxyR_kat1,2,3'
...
fig1.suptitle('WT in 0 H Model') #setting main title of fig
...
fig1.savefig('../figures/MHMnoC_WT_stat_dynamics')
    
    
    
    ###################
fig2,ax2 = plt.subplots(1,3, figsize=[8,5]) 
fig2.suptitle(('MHM Model Dynamics for '+str(st) )) #set main title 
...
fig2.savefig('../figures/'+str(st) +'_0H_odelib')
...
#print out plot
fig3.savefig('../figures/MHMnoC_'+str(st) +'_0_TRACE')
...
fig3.suptitle('Vibrio in 400 H Model') #setting main title of fig
...
fig4.suptitle('Vibrio Monoculture parameters in 400 HOOH ', fontsize = 16)
